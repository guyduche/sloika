...
def network(klen, sd, nfeature=4, winlen=3, fast_size=32, slow_size=32):
    ...
    fwd1 = smt.SCRN(insize, fast_size, slow_size, init=_prn, fun=rnn_act)
    bwd1 = smt.SCRN(insize, fast_size, slow_size, init=_prn, fun=rnn_act)
    layer1 = smt.birnn(fwd1, bwd1)

    layer2 = smt.FeedForward(2 * size, size, has_bias=True, fun=ff_act)

    fwd3 = smt.SCRN(size, fast_size, slow_size, init=_prn, fun=rnn_act)
    bwd3 = smt.SCRN(size, fast_size, slow_size, init=_prn, fun=rnn_act)
