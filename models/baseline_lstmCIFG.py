...
def network(klen, sd, nfeature=4, winlen=3, size=64):
    ...
    fwd1 = smt.LstmCIFG(insize, size, init=_prn, has_bias=True,
                       has_peep=True, fun=rnn_act)
    bwd1 = smt.LstmCIFG(insize, size, init=_prn, has_bias=True,
                       has_peep=True, fun=rnn_act)
    layer1 = smt.birnn(fwd1, bwd1)

    layer2 = smt.FeedForward(2 * size, size, has_bias=True, fun=ff_act)

    fwd3 = smt.LstmCIFG(size, size, init=_prn, has_bias=True,
                       has_peep=True, fun=rnn_act)
    bwd3 = smt.LstmCIFG(size, size, init=_prn, has_bias=True,
                       has_peep=True, fun=rnn_act)
